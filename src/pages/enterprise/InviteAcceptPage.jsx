...
import { CheckCircle2, AlertCircle, Building2, UserCircle2 } from 'lucide-react';
...
export default function EnterpriseInviteAcceptPage() {
  ...
  const accept = async () => {
    setSubmitting(true);
    try {
      await enterpriseService.acceptInvite(token);
      setPhase('done');
      setMessage('Bạn đã tham gia công ty thành công.');
    } catch (error) {
      setPhase('error');
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-ink-50 p-6">
      <div className="w-full max-w-md rounded-2xl bg-white p-8 text-center shadow-sm ring-1 ring-ink-100">
        ...
        {phase === 'confirm' && (
          <>
            <h1 className="mt-4 text-xl font-bold text-ink-700">Lời mời tham gia doanh nghiệp</h1>
            <p className="mt-2 text-sm text-ink-500">
              Bạn sẽ tham gia công ty với tài khoản dưới đây. Hãy kiểm tra đúng tài khoản trước khi
              xác nhận.
            </p>
